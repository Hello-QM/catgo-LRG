...
**安装 `catgo` Python 包。** 它自带完整应用（前端 Web UI 已打进 wheel）以及 CLI、MCP 服务和 skills；直接运行 `catgo` 即可启动应用。

```bash
pip install catgo                       # pip
uv tool install catgo                   # uv（独立工具）；或：uv pip install catgo
# conda / mamba 用户：先建环境，再 pip 安装
