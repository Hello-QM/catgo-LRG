...
test('canonical current release body contains every mandatory disclosure', () => {
  ...
  assert.match(body, /10\.1039\/D6DD00273K/)
...
test('existing release is always edited to the canonical body', () => {
  ...
  assert.match(calls, /^release view v1\.4\.14$/m)
  assert.doesNotMatch(calls, /^release create /m)
  assert.match(
    calls,
    /^release edit v1\.4\.14 --title CatGo v1\.4\.14 --notes-file \.github\/release-notes\/v1\.4\.14\.md$/m,
  )
})
...
test('concurrent draft creation still converges through an explicit edit', () => {
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout)
  assert.match(calls, /^release create v1\.4\.14 --draft /m)
  assert.match(calls, /^release view v1\.4\.14$/m)
  assert.match(calls, /^release edit v1\.4\.14 /m)
})
...
test('Tauri workflow finalizes the canonical body after tauri-action', () => {
  ...
  assert.match(block, /if: env\.CATGO_RELEASE_TAG == 'v1\.4\.14'/)
...
test('Tauri workflow retains the v1.4.8 canonical finalizer', () => {
  assert.match(
    WORKFLOW,
    /if: env\.CATGO_RELEASE_TAG == 'v1\.4\.8'[\s\S]*scripts\/ensure-release-body\.sh "\$CATGO_RELEASE_TAG"/,
  )
})

test('Tauri workflow retains the v1.4.9 canonical finalizer', () => {
  assert.match(
    WORKFLOW,
    /if: env\.CATGO_RELEASE_TAG == 'v1\.4\.9'[\s\S]*scripts\/ensure-release-body\.sh "\$CATGO_RELEASE_TAG"/,
  )
})
