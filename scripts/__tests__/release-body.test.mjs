...
test('existing release is always edited to the canonical body', () => {
  ...
  assert.match(calls, /^release view v1\.4\.9$/m)
  assert.doesNotMatch(calls, /^release create /m)
  assert.match(
    calls,
    /^release edit v1\.4\.9 --title CatGo v1\.4\.9 --notes-file \.github\/release-notes\/v1\.4\.9\.md$/m,
  )
})
...
test('concurrent draft creation still converges through an explicit edit', () => {
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout)
  assert.match(calls, /^release create v1\.4\.9 --draft /m)
  assert.match(calls, /^release view v1\.4\.9$/m)
  assert.match(calls, /^release edit v1\.4\.9 /m)
})
...
test('Tauri workflow finalizes the canonical body after tauri-action', () => {
  ...
  assert.match(block, /if: env\.CATGO_RELEASE_TAG == 'v1\.4\.9'/)
