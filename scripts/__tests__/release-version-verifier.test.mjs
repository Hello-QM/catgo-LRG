...
function replaceVersion(root, path, from = '1.4.14', to = '1.4.9') {
  const target = resolve(root, path)
...
test('accepts a consistent release tree and its exact v-prefixed tag', () => {
  const root = fixture()
  try {
    const result = runVerifier(root, '--tag', 'v1.4.14')
    assert.equal(result.status, 0, result.stderr)
    assert.match(result.stdout, /release version 1\.4\.14 verified/)
  } finally {
...
test('rejects a tag that does not exactly match the manifest version', () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0)
    assert.match(result.stderr, /expected v1\.4\.14/)
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
})
...
test('honors the workflow tag and publishing requirement environment', () => {
  ...
  try {
    ...
    assert.match(mismatched.stderr, /expected v1\.4\.14/)
