...
describe(`detect_bonds_reference`, () => {
  ...
  it(`rejects atoms beyond scaled covalent-radii sum`, () => {
  ...
  it(`uses multiplicative scale rather than a fixed absolute tolerance`, () => {
    ...
    // 3.5 Å is below 1.2 * (1.48 + 1.48) = 3.552 Å, so Rust atom_radii keeps it.
    // The old WebGPU formula, rsum + 0.45 = 3.41 Å, would incorrectly drop it.
  ...
  it(`finds a minimum-image bond across a periodic boundary`, () => {
    // cubic 5 Å cell; x=0.2 and x=4.7 are 4.5 apart direct, 0.5 across boundary
    const pos = new Float32Array([0.2, 0, 0, 4.7, 0, 0])
    const radii = new Float32Array([0.76, 0.76])
    const lat = new Float32Array([5, 0, 0, 0, 5, 0, 0, 0, 5])
    const bonds = detect_bonds_reference(pos, lat, radii, { ...OPTS, max_bond_dist: 3 })
    expect(bonds).toHaveLength(1)
    // Convention: jimage applied to b (b + jimage·L) reaches the min image of a.
    // b at 4.7, a at 0.2: dx = b-a = 4.5; shifting b by -a (na=-1 -> -5) gives
    // displacement -0.5, the shortest. So jimage = [-1,0,0].
    expect(bonds[0].jimage).toEqual([-1, 0, 0])
    // Lock the minimum-image distance the WGSL shader must reproduce: 5 - 4.5 = 0.5 Å.
    expect(bonds[0].dist).toBeCloseTo(0.5)
  })
