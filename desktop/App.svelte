<script lang="ts">
  ...
  let show_lab_link = $derived.by(() => {
    // Hide on non-structure tabs (workflow, terminal, etc.)
    if (tm.active_tab && tm.active_tab.type !== `structure`) return false
    const first_tab = tm.tabs.find(t => t.type === `structure`)
    if (!first_tab) return true
    const ts = tab_states[first_tab.id]
    if (!ts) return true
