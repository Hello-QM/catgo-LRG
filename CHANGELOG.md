...
- **Large systems get bonds again** — bond requests to the worker now carry a size-scaled deadline (a wedged worker used to hang the chain forever, leaving big structures bondless), and `solid_angle` auto-downgrades to `atom_radii` above 8,000 atoms (near-identical results at force-field scale, seconds instead of minutes).
