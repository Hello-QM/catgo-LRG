...
CatGo 1.4.11 is a cross-platform reliability release for CatBot, scientific
workflows, large trajectories, and the integrated terminal. It includes the
unpublished 1.4.10 improvements and adds packaged-runtime fixes verified on the
Windows, macOS Apple Silicon, and Linux build paths.

### Highlights

- **CatBot works from installed desktop packages** — Claude, Codex, and Gemini
  CLI discovery now follows platform-native user paths, while the bundled
  backend and Agent Bridge expose the same Provider, MCP, Skills, voice,
  structure, workflow, and Campaign capabilities as the source build.
...
- **Windows terminal and Campaign workflows are usable after installation** —
  local shell selection, working-directory synchronization, CLI discovery, and
  frozen Campaign subprocess launching now use the correct platform runtime.
...
- Generated surface workflows now contain an additional clean-slab relaxation
  node. Review slab constraints before submitting a newly generated DAG.
