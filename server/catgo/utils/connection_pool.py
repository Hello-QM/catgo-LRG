...
class HPCConnectionPool:
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while True:
            ...
            # Send keepalive pings to SubprocessSSHRunner connections
            # (asyncssh has its own keepalive_interval=60 set at connect time)
            for sid, hpc in list(self.connections.items()):
                if sid == LOCAL_SESSION_ID or not hpc.is_subprocess_mode:
                    continue
                try:
    ...
    async def try_reconnect(self, session_id: str) -> Optional[HPCConnection]:
        ...
        if auth == AuthMethod.SSH_CONFIG:
            try:
                ...
                hpc = await self.connect_ssh_config(config, session_id)
            ...
            try:
                ...
                hpc = await self.connect(config, session_id)
                self._dead_connections.pop(session_id, None)
                logger.info("[CatGo:HPC] Auto-reconnect succeeded for %s", session_id)
                return hpc
            except Exception as exc:
