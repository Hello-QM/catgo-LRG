...
async def submit_batch_tasks(
    ...
    # Lazy import to avoid circular dependency (submitter <-> batch_submitter)
    from catgo.workflow.engine.submitter import _get_workflow_config, _resolve_cluster_config

    # Load per-workflow config first (for session ID and base_work_dir)
    wf_config = _get_workflow_config(db, workflow_id)

    # Get HPC connection from the first task
    hpc = await get_hpc_connection(first_task, config, wf_config=wf_config)
    if not hpc:
    ...
    # Use first task ID fragment to disambiguate when multiple batches of the same type exist
    batch_suffix = first_task["id"][:8]
    batch_dir = f"{base_dir}/{workflow_id}/batch_{first_task['task_type']}_{batch_suffix}"
    ...
    await hpc.conn.run(f"mkdir -p {batch_dir}", check=True)
    ...
    for i, task in enumerate(tasks):
        ...
        await generator(hpc, work_dir, resolved_type, task_params, structure_str, config, task)

        # Upload custodian script if needed (same as _submit_one in submitter.py)
        if engine_key == "vasp":
            # Resolve per-cluster config for POTCAR and VASP command
            session_id = task.get("hpc_session_id") or wf_config.get("default_session_id", "")
            ...
            potcar_root = (
                ...
                or hpc_cfg.get("potcar_root", "")
            )
            potcar_func = (
                cluster_cfg.get("potcar_functional")
                or hpc_cfg.get("potcar_functional", "potpaw_PBE")
            )
            if potcar_root:
    ...
    # Resolve per-cluster config for job script
    session_id_0 = first_task.get("hpc_session_id") or wf_config.get("default_session_id", "")
    cluster_cfg_0 = _resolve_cluster_config(wf_config, session_id_0 or getattr(hpc, 'session_id', ''))

    # Generate a base job script from the first task
    # Priority: wf_config job_script_template > generated script
    ...
    if wf_template and "#SBATCH" in wf_template:
        ...
        vasp_cmd_0 = (
            cluster_cfg_0.get("vasp_command")
            or config.get("hpc", {}).get("run_commands", {}).get(engine_key)
            or _ENGINE_COMMANDS.get(engine_key, f"srun {engine_key}")
        )
        use_custodian = params.get("use_custodian",
            wf_config.get("use_custodian",
                config.get("hpc", {}).get("use_custodian", False)))
        if engine_key == "vasp" and use_custodian:
            vasp_cmd_0 = "python run_custodian.py"
        job_script = wf_template.rstrip() + f"\n\ncd $TASK_DIR\n{vasp_cmd_0}\n"
    else:
    ...
    result = await hpc.conn.run(f"cd {batch_dir} && sbatch submit_array.sh", check=False)
