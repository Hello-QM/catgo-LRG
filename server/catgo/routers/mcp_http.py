...
from catgo.routers.view_state import get_structure, push_structure, get_state_summary, get_selection_dict
...
async def _push_structure_direct(
    ...
    try:
        push_structure(struct, panel_id)
        return None
...
async def _handle_view_direct(client: httpx.AsyncClient, args: dict) -> list[TextContent]:
    ...
    if action == "get_state":
        return [T(type="text", text=_json.dumps(get_state_summary(), indent=2, ensure_ascii=False))]

    if action == "selection":
        return [T(type="text", text=_json.dumps(get_selection_dict(), indent=2, ensure_ascii=False))]
