...
async def run_install(req: InstallRequest) -> InstallResponse:
    ...
    try:
        ...
        if not os.path.isfile(script_path):
            ...
            result = await conn.run(
                "curl -fsSL https://raw.githubusercontent.com/Hello-QM/CatGO/dev/deploy/hpc/install-catgo.sh -o /tmp/install-catgo.sh"
            )
...
async def setup_claude_code(request: ClaudeCodeSetupRequest) -> dict:
    ...
    # SessionStart hook — detects CatGO backend + injects loaded structure info
...
'''
    ...
    # settings.json with SessionStart hook (idempotent — won't duplicate on re-run)
    settings_config = (
        '{"hooks":{"SessionStart":[{"matcher":"startup","hooks":'
        '[{"type":"command","command":"bash ~/.claude/hooks/catgo-session-start.sh","timeout":5}]}]}}'
    )
...
'''
        ...
        setup_cmd = (
            f"claude mcp add catgo --transport http {mcp_url} 2>/dev/null; "
            "mkdir -p ~/.claude/hooks && "
            # Save login node hostname for compute-node helper
            ...
            # Install compute-node helper script
            f"cat > ~/catgo-compute-setup.sh << 'COMPEOF'\n{compute_helper}COMPEOF\n"
            "chmod +x ~/catgo-compute-setup.sh"
        )

        # Idempotent settings.json: create if missing, skip if hook already registered
        setup_cmd += (
            ' && if [ ! -f ~/.claude/settings.json ]; then '
            f"echo '{settings_config}' > ~/.claude/settings.json; "
            'elif ! grep -q "catgo-session-start" ~/.claude/settings.json; then '
            f"echo '{settings_config}' > ~/.claude/settings.json; fi"
        )

        result = await conn.run(setup_cmd)
        if hasattr(result, "exit_status") and result.exit_status != 0:
            stderr = getattr(result, "stderr", "") or ""
            return {"success": False, "message": f"Failed to write config: {stderr}"}
