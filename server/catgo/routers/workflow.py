...
def api_run_workflow(workflow_id: str, config: WorkflowRunConfig):
    ...
    if config.execution_mode == "hpc" and not config.default_session_id:
        ...
        from workflow.node_sets import HPC_NODES
...
def _run_config_to_engine_config(config: WorkflowRunConfig) -> dict:
    ...
    # Resolve cluster-specific settings from the default session's cluster config
    cluster_cfg = config.cluster_configs.get(config.default_session_id)
    potcar_root = cluster_cfg.potcar_root if cluster_cfg else ""
    potcar_functional = cluster_cfg.potcar_functional if cluster_cfg else "potpaw_PBE"
    vasp_command = cluster_cfg.vasp_command if cluster_cfg else ""

    # Build job_defaults from default_job_params
    job_defaults = {}
    if config.default_job_params:
        jp = config.default_job_params
        if jp.nodes is not None:
            job_defaults["nodes"] = jp.nodes
        if jp.ntasks is not None:
            job_defaults["ntasks"] = jp.ntasks
        if jp.cpus_per_task is not None:
            job_defaults["cpus_per_task"] = jp.cpus_per_task
        if jp.walltime:
            job_defaults["walltime"] = jp.walltime
        if jp.partition:
            job_defaults["partition"] = jp.partition
        if jp.account:
            job_defaults["account"] = jp.account
    ...
    if vasp_command:
        hpc_cfg["run_commands"] = {"vasp": vasp_command}
    if config.job_script_template:
        hpc_cfg["job_script_template"] = config.job_script_template

    # Forward cluster_configs for per-session resolution
    if config.cluster_configs:
        hpc_cfg["cluster_configs"] = {
            sid: cc.model_dump() for sid, cc in config.cluster_configs.items()
        }
    ...
    # Also store at top level for submitter code that reads wf_config directly
    if config.job_script_template:
        result["job_script_template"] = config.job_script_template
    if config.cluster_configs:
        result["cluster_configs"] = {
            sid: cc.model_dump() for sid, cc in config.cluster_configs.items()
        }
    result["use_custodian"] = config.use_custodian
    return result
...
def api_list_steps(workflow_id: str):
    ...
    return [
        StepInfo(
            ...
            result_json=s.get("result_json") or "{}",
...
def api_get_step_results(workflow_id: str, step_id: str):
    ...
    try:
        ...
        with get_db() as conn:
            row = conn.execute(
                "SELECT result_json, node_type, status FROM workflow_steps WHERE id = ? AND workflow_id = ?",
                (step_id, workflow_id),
            ).fetchone()
        ...
        if row["status"] != "completed":
            raise HTTPException(status_code=400, detail=f"Step not completed (status: {row['status']})")
...
async def api_get_orca_progress(workflow_id: str, step_id: str):
    ...
    try:
        ...
        return {"points": points, "converged": orca_data.converged, "message": orca_data.message}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_get_irc_trajectory(workflow_id: str, step_id: str):
    """Download IRC trajectory file (IRC_Full_trj.xyz) from HPC on-demand.
    ...
    try:
        ...
        trajectory_path = f"{safe_work_dir}/IRC_Full_trj.xyz"

        try:
            # Read IRC trajectory file (likely 10-100 KB)
            content, _ = await read_remote_file(hpc.conn, trajectory_path, max_bytes=10 * 1024 * 1024)
            if not content:
                raise HTTPException(status_code=404, detail="IRC_Full_trj.xyz not found on HPC")

            return {
                "content": content,
                "filename": "IRC_Full_trj.xyz"
            }
        except asyncio.TimeoutError:
...
def api_preview_input(body: dict = Body(...)):
    ...
    try:
        if software == "orca":
            content = _preview_orca_input(node_type, params, structure_json)
        elif software == "cp2k":
...
def _preview_orca_input(node_type: str, params: dict, structure_json: str | None) -> str:
    ...
    basis = params.get("basis_set", params.get("basis", "6-31G"))
    charge = params.get("charge", 0)
    mult = params.get("multiplicity", 1)
    num_cores = params.get("num_cores", 4)

    opt_type_map = {
        "orca_sp": "SP", "orca_freq": "Freq", "orca_opt": "Opt",
        "single_point": "SP", "freq": "Freq", "geo_opt": "Opt",
    }
    opt_keyword = opt_type_map.get(resolved_type, opt_type_map.get(node_type, "Opt"))

    lines = [
        f"# ORCA input generated by CatGO workflow editor",
        f"# Node type: {node_type} → {resolved_type}",
        f"",
        f"! {method} {basis} {opt_keyword}",
        f"",
        f"%pal nprocs {num_cores} end",
        f"",
        f"* xyz {charge} {mult}",
    ]

    # Add coordinates from structure if available
    if structure_json:
        try:
            struct_dict = json.loads(structure_json) if isinstance(structure_json, str) else structure_json
            ...
            lines.append("  # Could not parse structure coordinates")
    else:
        lines.append("  # No structure available — add coordinates here")
