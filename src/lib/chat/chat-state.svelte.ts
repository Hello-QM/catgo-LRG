 ...
 *  Session list is now fetched from backend; this only tracks the active session.
 */
export function record_session(agent: string, session_id: string, _topic: string, _model?: string): void {
  // Ensure agent_sessions is up to date
  agent_sessions[agent] = session_id
}
