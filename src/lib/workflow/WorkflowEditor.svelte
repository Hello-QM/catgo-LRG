  ...
  async function load_workflow() {
    ...
    try {
      ...
      try {
        ...
        if (run_status.status !== `draft`) {
          exec.set_workflow_status(run_status.status)
          ...
          if (run_status.status === `running`) {
            ...
            start_monitoring()
          } else if (run_status.status === `paused`) {
            // Start monitoring if there are still running jobs — backend
            // watchers will broadcast their completion via WebSocket
            const has_active = run_status.steps.some(
              s => s.status === `running` || s.status === `queued` || s.status === `submitting`,
            )
            if (has_active) start_monitoring()
          }
          // Auto-select the most relevant node and show Status tab
          const priority = [`running`, `queued`, `submitting`, `failed`, `paused`]
          let active_id = ``
  ...
  <div class="center" bind:this={wf_wrapper_el}>
    ...
    {#if show_templates}
      <div class="tmpl-overlay">
        <div class="tmpl-title">Workflow Templates</div>
          ...
          <NodeStatusPanel
            ...
            onstatus_sync={(nid, db_status) => {
              ...
              if (node_statuses[nid] !== db_status) {
                ...
                const TERMINAL = new Set([`completed`, `not_converged`, `failed`, `skipped`, `pending`])
