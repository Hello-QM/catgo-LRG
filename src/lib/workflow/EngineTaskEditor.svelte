      ...
      <div class="ete-file-overlay">
        {#await import('$lib/structure/MonacoEditorPanel.svelte') then mod}
          <mod.default
            ...
            onclose={() => { show_file_editor = false }}
...
<style>
  ...
  /* ── File Editor Modal ── */
  .ete-file-overlay { position: fixed; inset: 0; z-index: 200; background: var(--page-bg, #0a0a0a); display: flex; flex-direction: column; }
  .ete-file-header { display: flex; align-items: center; justify-content: space-between; padding: 8px 16px; border-bottom: 1px solid var(--border-color, #333); background: var(--surface-bg, #111); flex-shrink: 0; }
  .ete-file-path { font-family: monospace; font-size: 13px; color: var(--text-color, #e5e5e5); }
  .ete-file-body { flex: 1; min-height: 0; display: flex; }
