<script lang="ts">
  ...
  let convergence = $state<{ points: ConvergencePoint[]; converged: boolean; message?: string } | null>(null)
  ...
  let irc_trajectory_content = $state<string | null>(null)
  ...
  const UNIFIED_CALC_TYPES = new Set([
    `geo_opt`, `single_point`, `cell_opt`, `md`, `freq`, `ts_search`,
  ])
  ...
  const MLP_TYPES = new Set([`mlp_relax`, `mlp_md`, `mlp_neb`, `mlp_vibrations`, `mlp_single_point`])
  ...
  interface CachedSummary {
    ...
    max_gradient?: number
    rms_gradient?: number
    frequencies?: Array<{ index: number; frequency_cm: number; imaginary: boolean }>
  ...
  const cached_summary = $derived.by<CachedSummary>(() => {
    if (!step_info?.result_json) return {}
    try { return JSON.parse(step_info.result_json) } catch (e) { console.warn(`[NodeStatusPanel] Failed to parse result_json for ${node_id}:`, e); return {} }
  })
  ...
  async function fetch_data(gen: number) {
    ...
    const has_run = effective_status === `running` || effective_status === `queued` || effective_status === `completed` || effective_status === `not_converged` || effective_status === `failed` || effective_status === `pending_review`
      ...
      if (conv_promise) {
        conv_promise.then(data => {
          ...
          if (is_restart) {
            ...
            convergence = { points: new_points, converged: data.converged ?? false, message: (data as any).message }
...
<div class="status-panel dialog-modal">
  ...
  <div class="sp-header">
    ...
    {#if display_status && display_status_color}
      <div class="sp-status-badge" style="background:{display_status_color}15;border-color:{display_status_color}40;color:{display_status_color}">
        <span class="sp-status-dot" class:running={effective_status === `running`} style="background:{display_status_color}"></span>
        {display_status}
      </div>
    {/if}
    {#if hpc_sid}
  ...
  {#if mode === 'task' && engine_task?.status === 'PENDING_REVIEW'}
    <div class="sp-section" style="background:rgba(245,158,11,0.1);border:1px solid #f59e0b;border-radius:8px;padding:12px;margin:8px 0;">
      ...
      <button
        style="background:#f59e0b;color:#000;border:none;border-radius:6px;padding:6px 16px;font-weight:600;cursor:pointer;"
        onclick={async () => {
          if (!task_id) return
      ...
      >
        {confirming_task ? 'Confirming...' : 'Confirm & Submit to HPC'}
      </button>
    </div>
  {/if}
  ...
  {:else}
    <div class="sp-body">
      ...
      {#if latest}
        <div class="sp-section">
          ...
          <div class="sp-info-grid">
            ...
            <div class="sp-info-row">
              <span class="sp-info-label">{is_orca ? 'dE (Eh)' : 'dE (eV)'}</span>
      ...
      {:else if is_orca && (cached_summary.energy_eh !== undefined || cached_summary.energy !== undefined || cached_summary.irc_converged !== undefined || cached_summary.transitions !== undefined) && status !== `running` && status !== `queued`}
        <div class="sp-section">
          <div class="sp-section-title">Results (cached)</div>
          <div class="sp-info-grid">
            <!-- Energy for all ORCA types -->
            <div class="sp-info-row">
            ...
            {#if node_type === `geo_opt` && is_orca}
              ...
              {#if cached_summary.max_gradient !== undefined}<div class="sp-info-row">
                <span class="sp-info-label">Max Gradient</span>
                <span class="sp-info-value mono">{cached_summary.max_gradient!.toFixed(6)}</span>
              </div>{/if}
              {#if cached_summary.rms_gradient !== undefined}<div class="sp-info-row">
            ...
            {#if node_type === `freq` && is_orca}
              ...
              {#if cached_summary.zpe_kj_mol !== undefined}<div class="sp-info-row">
                <span class="sp-info-label">ZPE (kJ/mol)</span>
                <span class="sp-info-value mono">{cached_summary.zpe_kj_mol!.toFixed(2)}</span>
              </div>{/if}
              {#if cached_summary.gibbs_eh !== undefined}<div class="sp-info-row">
                <span class="sp-info-label">G (Eh)</span>
                <span class="sp-info-value mono">{format_energy(cached_summary.gibbs_eh!)}</span>
              </div>{/if}
              {#if cached_summary.enthalpy_eh !== undefined}<div class="sp-info-row">
                <span class="sp-info-label">H (Eh)</span>
                <span class="sp-info-value mono">{format_energy(cached_summary.enthalpy_eh!)}</span>
              </div>{/if}
            ...
            <!-- ts_search (ORCA) -->
            {#if node_type === `ts_search` && (is_orca || is_mlp)}
              {#if cached_summary.activation_barrier_kcal_mol !== undefined}<div class="sp-info-row">
              ...
              {#if cached_summary.ts_imaginary_frequency !== undefined}<div class="sp-info-row">
              ...
              {#if status === `completed` || status === `not_converged` || status === `failed`}
                <div class="sp-info-row">
                  <button
                    ...
                    disabled={irc_trajectory_loading}
                  >
                    {irc_trajectory_loading ? `Loading...` : `View Trajectory`}
                  </button>
                </div>
              {/if}
            {/if}
        ...
        <!-- IRC trajectory viewer -->
        {#if node_type === `irc` && is_orca && irc_trajectory_content}
          <div class="sp-section">
            <div class="sp-section-title">Reaction Path Visualization</div>
            <div class="sp-trajectory-viewer">
              <Trajectory
                data_url={`data:chemical/x-xyz;base64,${btoa(irc_trajectory_content)}`}
              />
            </div>
          </div>
        {/if}
        ...
        <div class="sp-section">
          ...
          {#if node_type === `irc`}
            <div class="sp-ssh-hint">
              {loading ? `Fetching IRC progress...` : convergence?.message || `IRC in progress...`}
            </div>
          {:else if node_type === `uvvis`}
            <div class="sp-ssh-hint">
              {loading ? `Fetching TD-DFT progress...` : convergence?.message || `Computing excited states...`}
            </div>
          {:else}
