...
export function create_workflow_execution(tab_id: string = `default`): WorkflowExecution {
  ...
  async function check_stale_running_impl() {
    ...
    try {
      ...
      const ACTIVE = new Set([`running`, `queued`, `submitting`])
      for (const s of steps) {
        if (ACTIVE.has(node_statuses[s.id]) && s.status !== node_statuses[s.id]) {
          node_statuses = { ...node_statuses, [s.id]: s.status }
        }
      }
      const still_running = Object.values(node_statuses).some(s => ACTIVE.has(s))
      if (!still_running && sim_running) {
        sim_running = false
