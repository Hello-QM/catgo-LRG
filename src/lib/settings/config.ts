...
export const SETTINGS_CONFIG: SettingsConfig = {
  ...
  structure: {
    ...
    atom_radius: {
      value: 1.5,
      description: `Radius multiplier for atoms (1.5 = default, 0.5 = VESTA ball-and-stick, 1.0 = standard atomic radii)`,
      minimum: 0.1,
      maximum: 3.0,
    },
    same_size_atoms: {
    ...
    show_bonds: {
      ...
      enum: Object.fromEntries(
        [`never`, `always`, `crystals`, `molecules`].map((key) => [key, key[0].toUpperCase() + key.slice(1)]),
    ...
    incomplete_periodic_edge_mode: {
      value: false,
      description: `Render cross-cell bonds as a single stub on atom A's side instead of paired stubs (VESTA Mode 1)`,
    },
    incomplete_edge_length_scale: {
      value: 0.5,
      description: `Length of the visible stub for cross-cell bonds in incomplete-edge mode (fraction of half-bond length)`,
      minimum: 0.05,
      maximum: 1.0,
    },
    bonding_strategy: {
    ...
    depth_cueing: {
      value: 0.4,
      description: `Depth cueing (3Dmol-style fog) intensity (0 = off, 1 = maximum). Fades distant atoms toward the background color. Near/far auto-tracked from camera distance.`,
      minimum: 0,
      maximum: 1,
    },
    depth_cue_start: {
      value: 0.4,
      description: `Where fog begins, as fraction of structure depth (0 = front face, 1 = back face). 3Dmol default is 0.4 — front 40% of atoms remain crisp, only the back fades.`,
      minimum: 0,
      maximum: 1,
    },
    depth_cue_end: {
      value: 1,
      description: `Ending depth for depth cueing as fraction of structure extent (0 = front face, 1 = back face). Atoms beyond this are fully faded.`,
      minimum: 0,
      maximum: 1,
    },
    ...
    polyhedra_metals_only: {
      value: true,
      description: `Only show polyhedra around metal atoms (uncheck to include non-metals)`,
    },
    polyhedra_cutoff: {
      value: 3.5,
    ...
    clip_active: {
      value: false,
      description: `Enable sphere clipping to show only atoms within a radius of a selected atom`,
    },
    clip_radius: {
      value: 8,
  ...
  gesture: {
    ...
    voice_method: {
      value: `auto`,
      description: `Voice recognition engine: auto tries Web Speech API first then Whisper`,
      enum: {
    ...
    whisper_api_key: {
      value: ``,
      description: `OpenAI API key for Whisper voice recognition (only used when Whisper method is active)`,
      context: `web`,
    },
    art_trail_spacing: {
      value: 2.0,
    ...
    tts_enabled: {
      value: true,
      description: `Enable voice response (text-to-speech) for gesture commands and AI answers`,
      context: `web`,
    },
    tts_volume: {
      value: 0.8,
    ...
    voice_ai_enabled: {
      value: true,
      description: `Route unrecognized voice commands to AI chat for complex operations (requires API key)`,
      context: `web`,
    },
  },
}
