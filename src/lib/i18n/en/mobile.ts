...
const mobile: Record<string, string> = {
  ...
  save_pw_body:
    `Next time you connect to {user} you'll only need the one-time passcode (OTP). The password is encrypted on this device.`,
  save_pw_not_now: `Not now`,
  save_pw_save: `Save password`,
  save_pw_retry: `Retry`,
  save_pw_failed:
    `Couldn't save the password — it wasn't stored. Retry, or continue and enter it next time.`,
  ...
  ks_body:
    `Generate an SSH key on this device and install it on {user}. Future connects can use the key instead of a password. The private key is generated on-device and stored encrypted — it never leaves your phone unprotected.`,
  ...
  ai_setup: `AI setup`,
  ai_setup_subtitle:
    `Bring your own API key — stored encrypted on this device, never uploaded.`,
  ai_api_key: `API key`,
  ai_api_key_placeholder: `Paste your API key`,
  ai_key_saved: `API key saved`,
  ai_replace_key: `Replace`,
  ...
  ai_rate_limited:
    `Rate limit reached — the free tier allows only a few messages per minute. Wait a few seconds and resend, or pick a higher-limit model in settings.`,
  ai_model_busy:
    `The model is busy right now (high demand) — usually temporary. Wait a moment and resend, or try a different model in settings.`,
