<script lang="ts">
  ...
  // Phase 7 — image-atom decorator layout. When the parent has produced
  // ghost image atoms (`num_original_sites !== undefined`), enumerate the
  // crystaltoolkit-style sites_to_draw set on the pre-ghost structure and
  // build the CSR mapping from each non-home image atom to the BondManager
  // slots touching its anchor. Empty layout for molecules / ghosts-off /
  // body-centered structures with no boundary atoms — the renderer's
  // decorator pass becomes a no-op.
  let sites_to_draw = $derived.by((): Map<ImageSiteKey, ImageSiteEntry> | null => {
    if (num_original_sites === undefined) return null
  ...
  $effect.pre(() => {
    ...
    const conn = bond_state.bond_connectivity
    ...
    if (traj_positions != null) {
      // Skip if neither connectivity nor positions changed since last fire.
      // This guard mostly helps under Phase 4 when struct_ref and
      // current_structure become stable; before Phase 4, struct_ref still
      // changes per frame so this guard rarely hits.
      if (
