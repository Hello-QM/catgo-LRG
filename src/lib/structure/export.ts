...
// Derive a CIF-safe `data_<name>` block name from a structure. Shared by the
// periodic and Cartesian (molecule) CIF writers so both label blocks the same way.
function cif_data_name(structure: AnyStructure): string {
  return (
...
// Generate a Cartesian CIF (no unit cell) for a structure with no lattice —
// i.e. an isolated molecule / cluster. Uses `_atom_site_Cartn_*` columns so
// the coordinates round-trip as absolute Cartesian positions; no
// `_cell_length`/`_cell_angle` block is emitted.
function cartesian_cif_str(structure: AnyStructure): string {
  const lines: string[] = []

  lines.push(`# CIF file generated by CatGo`)
  lines.push(`data_${cif_data_name(structure)}`)
  lines.push(``)
  ...
  for (let idx = 0; idx < structure.sites.length; idx++) {
    ...
    // Extract element symbol + occupancy the same way the periodic loop does.
    let element_symbol = `X` // default fallback
    let occupancy = 1
    if (
      site.species &&
    ...
    // Cartesian coords: prefer site.xyz, fall back to site.abc only if absent.
    const cart = (site.xyz && Array.isArray(site.xyz) && site.xyz.length >= 3)
      ? site.xyz
      : site.abc
    if (!cart || !Array.isArray(cart) || cart.length < 3) {
      throw new Error(`No valid coordinates found for site ${idx}`)
    }

    const label = site.label || `${element_symbol}${idx + 1}`
    lines.push(
      `${label} ${element_symbol} ${cart[0].toFixed(8)} ${
        cart[1].toFixed(8)
      } ${cart[2].toFixed(8)} ${occupancy.toFixed(8)}`,
    )
  }
...
export function structure_to_cif_str(structure?: AnyStructure): string {
  if (!structure?.sites) throw new Error(`No structure or sites to export`)
  // No lattice → write a Cartesian (molecule/cluster) CIF instead of refusing.
  if (!(`lattice` in structure) || !structure.lattice) {
    return cartesian_cif_str(structure)
  }
