<script lang="ts">
  ...
  // MCP 轮询桥 — 三个异步循环: 截图捕获(2s) + 结构信息推送(5s) + 待更新结构拉取(500ms)
  //
  ...
  $effect(() => {
    return untrack(() => start_mcp_bridge(mcp_bridge_deps))
  })
  ...
  function handle_optimade_preview(optimade_struct: any, pymatgen_struct: PymatgenStructure) {
    ...
    // Try to get provider name from the structure or use fallback
    optimade_pending_provider = optimade_struct.attributes?.database_provider ?? 'OPTIMADE'

    optimade_preview_visible = true
  }

  // Confirm and execute OPTIMADE import
  function confirm_optimade_import() {
    console.log('[OPTIMADE] Confirm import clicked, pending structure:', optimade_pending_pymatgen)
    if (optimade_pending_pymatgen) {
      console.log('[OPTIMADE] Calling handle_optimade_import with structure:', optimade_pending_pymatgen)
      handle_optimade_import(optimade_pending_pymatgen)
      console.log('[OPTIMADE] Import completed, closing modals')
      ...
      console.warn('[OPTIMADE] No pending pymatgen structure to import')
    }
  }

  // Cancel OPTIMADE import and close preview
  function cancel_optimade_import() {
    // Close preview, keep search modal open
    optimade_preview_visible = false
    optimade_pending_structure = null
    optimade_pending_pymatgen = null
    optimade_pending_provider = null
