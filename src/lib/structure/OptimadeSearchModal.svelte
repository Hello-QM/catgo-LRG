<script lang="ts">
  ...
  import type { OptimadeProvider, OptimadeSearchResult, OptimadeStructure, ProviderDetails } from '$lib/api/optimade'
  import {
  ...
  import type { PubChemSearchResponse, PubChemSearchCompound } from '$lib/api/pubchem'
  ...
  let { visible, onclose, onimport, onpreview, initial_elements = ``, initial_provider = `` }: Props = $props()
  ...
  async function fetch_mp_summaries() {
    ...
    try {
      ...
      const new_map = new Map<string, MPSummaryData>()
  ...
  async function compute_structure_details(structures: OptimadeStructure[]) {
    ...
    const new_map = new Map<string, ComputedDetails>()
  ...
  async function do_search(page = 0) {
    if (!search_formula && !search_elements) return
    ...
    if (is_pubchem) {
      ...
      try {
        const elements = search_elements
          .split(/[,\s]+/)
          .map((e) => e.trim())
          .filter((e) => e.length > 0)

        pubchem_results = await search_pubchem_compounds(
          search_formula || undefined,
    ...
    try {
      ...
      search_results = await search_optimade_structures(selected_provider, providers, {
        formula: search_formula || undefined,
      ...
      if (page === 0 && search_results?.structures?.length === 0 && search_formula && /\d/.test(search_formula)) {
    ...
    <div class="modal-content" bind:this={modal_element} role="dialog" aria-modal="true">
      ...
      <div class="modal-body">
        ...
        <div class="periodic-table-section">
          <p class="pt-hint">Click an element to search for structures containing it:</p>
          <PeriodicTable
            tile_props={{
              onclick: ({ element }) => {
                search_elements = element.symbol
                search_formula = ``
                do_search()
              },
            }}
