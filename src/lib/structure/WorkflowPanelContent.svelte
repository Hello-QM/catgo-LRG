<script lang="ts">
  /** 工作流面板内容 — 唯一内容组件, 停靠/悬浮/每-pane 壳共用。
   * 不感知自己处于哪种模式; 目标解析一律经冻结的 target
   * (resolve_target_structure), 禁读全局 current。 */
  import { API_BASE } from '$lib/api/config'
  import type { AnyStructure } from '$lib/structure'
  import { t, load_i18n_module } from '$lib/i18n/index.svelte'
  import { viewer_manifests_state } from '$lib/structure/viewer-registry.svelte'
  import {
    overlay_log,
  ...
  let { target = null, active = true, fallback_structure, on_open_workflow_editor }: {
    target?: OverlayTargetContext | null
    /** 宿主可见时才轮询 */
    active?: boolean
    /** 无注册表句柄时的后备结构 (每-pane 壳传入) */
    fallback_structure?: AnyStructure
    on_open_workflow_editor?: (workflow_id: string) => void
  } = $props()

  const reg = viewer_manifests_state()
  const target_ok = $derived(validate_target(target).ok)
  const target_name = $derived(
    target?.viewport_id && target_ok
      ? reg.manifests[target.viewport_id]?.filename ??
        reg.manifests[target.viewport_id]?.label ?? null
      : null,
  )
  const can_create = $derived(target ? target_ok : !!fallback_structure)

  /** 冻结目标解析: 本面板绑定视口的活结构 → 后备 prop; 绝不读全局 current */
  function get_target_structure(): AnyStructure | null {
    const live = resolve_target_structure(target)
    if (live?.sites?.length) return live
    return fallback_structure ?? null
  }
...
<div class="workflow-panel-content">
      ...
      <div class="section-box new-section">
        <button
          ...
          onclick={create_workflow_with_structure}
          disabled={creating || !can_create}
          title={target_name ?? undefined}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M12 5v14M5 12h14" />
          </svg>
          <span class="new-workflow-label">
            {creating
              ? t('structure.creating')
              : target_name
              ? t('structure.workflow_new_with_named', { name: target_name })
              : can_create
              ? t('structure.workflow_new_with_structure')
              : t('structure.workflow_new')}
          </span>
        </button>
        {#if !can_create}
          <div class="wf-target-hint">
            {target
              ? t('structure.workflow_no_target_structure')
              : t('common.overlay_pick_target_first')}
          </div>
        {/if}
      </div>
