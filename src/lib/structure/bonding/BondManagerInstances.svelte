<script lang="ts">
  ...
  // Lattice matrix, incomplete-edge mode, or image-atom layout changes
  // invalidate the entire bond-instance buffer (cross-cell transforms and
  // decorator instances both reference these inputs). Force a full resync
  // — incremental updates only refresh dirty BondManager slots.
  $effect(() => {
    void lattice_matrix
  ...
  let last_atom_colors_ref: Float32Array | undefined
  let last_count_seen = 0
  $effect(() => {
    if (!renderer) return
    const mgr = bond_manager
    const ac = atom_colors
    // Track topology changes — when bonds change, colors might need to move slot.
    const _version = mgr.version
    void _version

    // When atom_colors reference changes (e.g. property_colors switched),
    // all bonds' colors need to be rewritten — force every slot dirty.
    const ac_changed = ac !== last_atom_colors_ref
    last_atom_colors_ref = ac

    const count = mgr.count
    if (count === 0 || ac.length === 0) {
      last_count_seen = count
      return
    }

    // We CANNOT rely on mgr.dirty_slots / dirty_all here: BondInstancedRenderer's
    // sync $effect runs in the same Svelte flush and may have already called
    // mgr.clear_dirty() before this effect fires. Track last_count_seen
    // independently to detect newly-added slots, and rewrite all colors when
    // atom_colors switches.
    const count_grew = count > last_count_seen
    last_count_seen = count
    ...
    mgr.ensure_colors()
    try {
      // Always rewrite all colors. count is bounded (typically <10k) so the
      // per-update cost is microseconds. Simpler than fighting effect ordering.
      // Skip the loop body only when nothing changed to avoid version churn.
      if (ac_changed || count_grew || mgr.dirty_all) {
        for (let slot = 0; slot < count; slot++) {
          write_slot_color(slot, pairs, ac, mgr)
        }
      } else {
        // Even with no growth, the live dirty_slots set (if not yet cleared
        // by sync) holds slots whose pair changed (rare, e.g. set_pair).
        for (const slot of mgr.dirty_slots) {
          if (slot < count) write_slot_color(slot, pairs, ac, mgr)
        }
      }
    } finally {
  ...
  function write_slot_color(
    ...
    const a3 = a * 3
    const b3 = b * 3
    if (a3 + 2 >= ac.length || b3 + 2 >= ac.length) return
    mgr.set_colors(
      slot,
      ac[a3],     ac[a3 + 1], ac[a3 + 2],
      ac[b3],     ac[b3 + 1], ac[b3 + 2],
    )
  }
