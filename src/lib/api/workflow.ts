...
/** Get live ORCA optimization progress from ORCA.out */
...
export function connect_workflow_monitor(workflow_id: string, callbacks: {
  on_step_status?: (step_id: string, status: string, job_id?: string) => void
  ...
  import(`@tauri-apps/api/event`).then(({ listen }) => {
    listen<{ type: string; step_id?: string; status?: string; message?: string }>(
      ...
      (event) => {
        ...
        switch (data.type) {
          case `step_status`:
            callbacks.on_step_status?.(data.step_id!, data.status!)
            break
    ...
    ws.onmessage = (event) => {
      ...
      switch (msg.type) {
        ...
        case `step_status`:
          ...
          callbacks.on_step_status?.(msg.step_id, msg.status, msg.job_id)
          break
        case `workflow_status`:
