...
import { worktree_offset, server_port, shared_define, json_gz_plugin } from './vite.shared'
...
if (!existsSync(_sk_tsconfig)) {
  ...
  writeFileSync(_sk_tsconfig, JSON.stringify({
    compilerOptions: {
      module: `esnext`,
...
export default defineConfig({
  ...
  plugins: [
    ...
    } satisfies Plugin,
    {
      ...
      transform(code, id) {
        if (id.includes(`ferrox-wasm.ts`)) {
          const wasm_path = resolve(__dirname, `extensions/rust-wasm/pkg/ferrox_bg.wasm`).replace(/\\/g, `/`)
        ...
        server.middlewares.use((req, res, next) => {
          ...
          if (url.pathname === `/__db/browse`) {
            ...
            if (raw === `__drives__` && platform() === `win32`) {
              try {
                ...
                res.end(JSON.stringify({ dir: `__drives__`, parent: `__drives__`, items }))
              } catch {
            ...
            if (!existsSync(target) || !statSync(target).isDirectory()) {
              res.statusCode = 400; res.end(`not a directory`); return
            }
            ...
            const items = entries
              .filter(e => e.isDirectory() || DB_EXTS.has(extname(e.name).toLowerCase()))
              .map(e => ({
                name: e.name,
            ...
            const parent_val = (platform() === `win32` && parent === target) ? `__drives__` : parent
          ...
          if (url.pathname === `/__files/read`) {
            ...
            try {
              ...
              res.statusCode = 400; res.end(`cannot read file`)
            }
            return
          }
          ...
          if (url.pathname === `/__files/write` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                res.statusCode = 500; res.end(`write failed: ${e}`)
              }
            })
            return
          }
          ...
          if (url.pathname === `/__files/mkdir` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                res.statusCode = 500; res.end(JSON.stringify({ success: false, message: `${e}` }))
              }
            })
            return
          }
          ...
          if (url.pathname === `/__files/delete` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                if (parts.length < 2) {
                  res.statusCode = 400; res.end(JSON.stringify({ success: false, message: `Path too shallow` })); return
                }
                if (!existsSync(target)) { res.statusCode = 404; res.end(JSON.stringify({ success: false, message: `Not found` })); return }
                const { rmSync } = require(`fs`)
                rmSync(target, { recursive: true, force: true })
                res.setHeader(`Content-Type`, `application/json`)
                res.end(JSON.stringify({ success: true, message: `Deleted ${basename(target)}` }))
              } catch (e) {
                res.statusCode = 500; res.end(JSON.stringify({ success: false, message: `${e}` }))
              }
            })
          ...
          if (url.pathname === `/__files/rename` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                if (!existsSync(src)) { res.statusCode = 404; res.end(JSON.stringify({ success: false, message: `Not found` })); return }
                renameSync(src, dst)
                res.setHeader(`Content-Type`, `application/json`)
                res.end(JSON.stringify({ success: true, message: `Renamed to ${basename(dst)}` }))
              } catch (e) {
                res.statusCode = 500; res.end(JSON.stringify({ success: false, message: `${e}` }))
              }
            })
          ...
          if (url.pathname === `/__files/copy` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                res.setHeader(`Content-Type`, `application/json`)
                res.end(JSON.stringify({ success: true, message: `Copied to ${basename(dst)}` }))
              } catch (e) {
                res.statusCode = 500; res.end(JSON.stringify({ success: false, message: `${e}` }))
              }
            })
          ...
          if (url.pathname === `/__files/move` && req.method === `POST`) {
            ...
            req.on(`end`, () => {
              try {
                ...
                renameSync(src, dst)
                res.setHeader(`Content-Type`, `application/json`)
                res.end(JSON.stringify({ success: true, message: `Moved to ${basename(dst)}` }))
              } catch (e) {
                res.statusCode = 500; res.end(JSON.stringify({ success: false, message: `${e}` }))
              }
            })
          ...
          if (url.pathname === `/__db/copy` && req.method === `POST`) {
            ...
            if (!existsSync(src)) { res.statusCode = 404; res.end(`source not found`); return }
    ...
    // WebSocket with the same JSON protocol as the Python backend.
    {
      ...
      configureServer(server) {
        // ====== Shell detection ======
        interface ShellInfo { id: string; label: string; command: string; args: string[] }

        function detect_shells(): ShellInfo[] {
          const shells: ShellInfo[] = []
          if (platform() === `win32`) {
            // PowerShell (always available on Windows)
            shells.push({ id: `powershell`, label: `PowerShell`, command: `powershell.exe`, args: [`-NoLogo`] })
            // PowerShell 7 (pwsh)
            try { execFileSync(`pwsh.exe`, [`-Version`], { stdio: `ignore`, timeout: 3000 }); shells.push({ id: `pwsh`, label: `PowerShell 7`, command: `pwsh.exe`, args: [`-NoLogo`] }) } catch {}
            // Git Bash
            ...
            for (const p of git_bash_paths) {
              if (existsSync(p)) { shells.push({ id: `git-bash`, label: `Git Bash`, command: p, args: [`--login`] }); break }
            }
            // Also check PATH for bash.exe (Git Bash or MSYS2)
            if (!shells.some((s) => s.id === `git-bash`)) {
              try { execFileSync(`bash.exe`, [`--version`], { stdio: `ignore`, timeout: 3000 }); shells.push({ id: `git-bash`, label: `Git Bash`, command: `bash.exe`, args: [`--login`] }) } catch {}
            }
            // Command Prompt
            shells.push({ id: `cmd`, label: `Command Prompt`, command: `cmd.exe`, args: [] })
            // WSL
            try { execFileSync(`wsl.exe`, [`--status`], { stdio: `ignore`, timeout: 3000 }); shells.push({ id: `wsl`, label: `WSL`, command: `wsl.exe`, args: [] }) } catch {}
          } else {
            // Unix: check common shells
            const user_shell = process.env.SHELL || `/bin/bash`
            const user_shell_name = basename(user_shell)
            shells.push({ id: user_shell_name, label: user_shell_name, command: user_shell, args: [`-l`] })
            // Add other known shells if present and not already the user shell
            for (const [id, path] of [[`bash`, `/bin/bash`], [`zsh`, `/bin/zsh`], [`fish`, `/usr/bin/fish`]] as const) {
        ...
        server.middlewares.use((req, res, next) => {
          if (req.url === `/api/pty/shells`) {
            res.setHeader(`Content-Type`, `application/json`)
            res.end(JSON.stringify(available_shells.map((s) => ({ id: s.id, label: s.label }))))
            return
          }
          next()
        })
              ...
              try {
                ...
                if (msg.action === `open`) {
                  ...
                  const shell_info = (requested_shell_id
                    ? available_shells.find((s) => s.id === requested_shell_id)
